...
# Create distribution data 
x1_distribution_data = as.data.frame(cbind(x1_seq, y1_seq)) %>% 
  mutate(data_seq_1 = x1_seq*y1_seq)
...
# Start function 
saveGIF_simple(for (se2_3 in seq(.5,1,length.out = 80)) {
  
  # Initialited data to construct the distribution 
  # Construct a standard normal distribution 
  y2_seq = dnorm(x2_seq, mean = x2_mean,sd = se2_3)
  y3_seq = dnorm(x = x3_seq,mean = x3_mean,sd = se2_3)
