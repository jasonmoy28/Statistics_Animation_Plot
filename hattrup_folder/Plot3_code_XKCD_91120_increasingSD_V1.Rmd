...
saveGIF_simple(for (moving_x in seq(65,130,length.out = 80)) {
  ...
  print(plot(plot_))},
  movie_name = 'Plot3_moving_X_with_z_score_XKCD82220_V1_NEGZb.gif', 
...
text_X_position = 57
mean = 75
saveGIF_simple(loop_object = for (sd in seq(5,20,length.out = 80)) {
  y = dnorm(x,mean = mean,sd = sd)
  plot2_data_in_loop = as.data.frame(cbind(x, y))
  ...
  plot_ = plot2_data_in_loop %>% 
    ggplot(data =.,aes(x = x,y = y)) + 
    geom_line(color = 'blue', size = 2) +
    geom_line(data = data.frame(x = c(fix_X,fix_X), y = c(0,0.10)),color = 'red', size = 2) + 
  ...
  print(plot(plot_))},
  movie_name = 'Plot3_ZwithincreasingSD_XKCD91120_V1.gif', 
  clone_repository_path = clone_repository_path,
  loop_speed = 0.01, loop_number = 1)
```